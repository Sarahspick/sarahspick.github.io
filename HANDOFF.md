...
- 해시태그는 인스타 제한으로 게시물당 최대 5개. 4개는 주제 태그, 1개는 #affiliate. #sarahspick 브랜드 태그는 자리가 없어 뺐음 (2026-09-16).
...
- 저장소에 있는 것: index.html(사이트), build_batch2.py, build_site2.py, catalog.json, thumbs/01.jpg~09.jpg(사이트에 박힌 썸네일과 동일한 바이트), profile.jpg, Buffer Document.md, 이 문서.
...
- 문구만 다시 만들기: `python3 build_batch2.py --copy-only` (ffmpeg, Pillow 불필요) → out/Buffer Document.md. 게시 순서는 build_batch2.py 상단 SCHEDULE(슬롯 → 날짜)이 기준이고, catalog.json의 publish_day와 같이 맞춘다.
- 영상부터 재생성 (새 배치): 영상 파일들이 있는 폴더에서 `python3 build_batch2.py` (ffmpeg, Pillow 필요. out/에 catalog_embedded.json, catalog.json, Buffer Document.md, thumbs/ 생성) → out/ 폴더에서 `python3 build_site2.py` (profile.jpg 필요) → index.html. catalog_embedded.json이 있으면 그걸 우선 읽는다.
...
- 완료: 슬롯 5 캡션의 "look for" 이름을 사이트 제목과 통일(Buffer Document.md, build_batch2.py).
...
3. Create Post → 인스타 채널 선택 → 영상 업로드 → 형식 Reel → Buffer Document.md의 "인스타 캡션" 블록을 그대로 붙여넣기 → Add to Queue (자동으로 다음 09:00 슬롯에 들어감). 날짜를 직접 정하고 싶으면 Schedule Post에서 09-17 09:00, 09-18 09:00... 순서대로.
